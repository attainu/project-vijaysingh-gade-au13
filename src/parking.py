...
class Parking(object):
    ...
    def create_parking_lot(self, no_of_slots):
        ...
        if no_of_slots > 0:
            for i in range(1, no_of_slots+1):
                temp_slot = lot.PSlot(slot_no=i, available=True)
    ...
    def get_nearest_available_slot(self):
        ...
        for slot in self.slots.values():
            if not slot.available:
                total_unaviliable += 1
        if total_unaviliable == len(self.slots):
            return False
        else:
    ...
    def status(self):
        ...
        pt = PrettyTable()
        pt.field_names = ["Slot No", "Registration No", "Colour"]
        for i in self.slots.values():
            if not i.available and i.car:
                pt.add_row([i.slot_no, i.car.reg_no, i.car.colour])
        print(pt)
